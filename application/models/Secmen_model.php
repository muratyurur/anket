...
class Secmen_model extends CI_Model
{
    /** Defining Table Name */
    ...
    public function get_records($where = array(), $limit, $count, $order = "mahalle ASC, sandik_alani ASC, sandik_no ASC, sandik_sira ASC")
    {
    ...
    {
        return $this->db->select("distinct(kapi)")->where($where)->get($this->tableName)->num_rows();
    }

    /** The method of returning count of records */
    public function get_hane($where = array())
    {
        return $this->db->select("distinct(kapi), daire")->where($where)->get($this->tableName)->num_rows();
    }
}